-- Separa el interes financiero, el interes moratorio y otros conceptos en
-- Ingresos Campo Santo sin modificar el total contable de cada recibo.

-- Los bloques condicionales permiten reintentar la migracion si el cliente SQL
-- llego a confirmar alguno de los renombramientos antes de un error posterior.
DO $$
BEGIN
  IF to_regclass('public.vw_ingresos_campo_santo_contrato_base_pre_mora') IS NULL THEN
    IF to_regclass('public.vw_ingresos_campo_santo_contrato_base') IS NULL THEN
      RAISE EXCEPTION
        'No existe la vista vw_ingresos_campo_santo_contrato_base requerida por la migracion';
    END IF;

    EXECUTE
      'ALTER VIEW public.vw_ingresos_campo_santo_contrato_base '
      'RENAME TO vw_ingresos_campo_santo_contrato_base_pre_mora';
  END IF;
END;
$$;
...
BEGIN
  IF EXISTS (
    ...
    WHERE table_schema = 'public'
      AND table_name = 'vw_ingresos_campo_santo_contrato_base_pre_mora'
      AND column_name = 'otros_cobrados_historico'
  ) THEN
    EXECUTE
      'ALTER VIEW public.vw_ingresos_campo_santo_contrato_base_pre_mora '
      'RENAME COLUMN otros_cobrados_historico TO otros_cobrados_historico_legacy';
  ELSIF NOT EXISTS (
    SELECT 1
    FROM information_schema.columns
    WHERE table_schema = 'public'
      AND table_name = 'vw_ingresos_campo_santo_contrato_base_pre_mora'
      AND column_name = 'otros_cobrados_historico_legacy'
  ) THEN
    RAISE EXCEPTION
      'La vista base historica no contiene la columna otros_cobrados_historico esperada';
  END IF;
END;
$$;
...
COMMENT ON VIEW public.vw_ingresos_campo_santo_movimiento_desglose IS
  'Clasifica cada pago aplicado una sola vez como capital, interes financiero, mora, otros o mantenimiento.';
    ...
    AND (
      ...
      OR (
        ...
        AND (
          ...
          OR base.mantenimiento_vencidas > 0
        )
      )
      OR (
        NOT p_solo_con_vencidas
        AND base.total_vencido_con_mora <= 0
        AND base.mantenimiento_vencidas = 0
      )
    )
    AND (
    ...
    SELECT
      ...
      COUNT(*) FILTER (
        WHERE filtrado.total_vencido_con_mora > 0
          OR filtrado.mantenimiento_vencidas > 0
